// ============================================================
// B3: B2 + 코어 고정 + OpenMP FC 병렬화
//
// B2 대비 변경:
//  + S0 → core {0}
//  + S1 → core {1}
//  + S2 → core {2,3,4,5} (OpenMP 4 스레드)
//  + run_fc_cpu에 #pragma omp parallel for 추가
//
// 유지:
//  - pthread_mutex + cond_var (B4에서 futex로 교체)
// ============================================================
...
enum SlotState { EMPTY, INPUT_READY, CONV_DONE };
...
static void slot_init() {
    for (int s = 0; s < N_SLOTS; s++) {
        pthread_mutex_init(&g_slots[s].mtx, nullptr);
        pthread_cond_init (&g_slots[s].cv,  nullptr);
        g_slots[s].state = EMPTY;
        memset(g_slots[s].input_buf, 0, sizeof(g_slots[s].input_buf));
        memset(g_slots[s].pool3_buf, 0, sizeof(g_slots[s].pool3_buf));
    }
}
static void slot_wait_until(Slot& s, SlotState target) {
    pthread_mutex_lock(&s.mtx);
    while (s.state != target)
        pthread_cond_wait(&s.cv, &s.mtx);
    pthread_mutex_unlock(&s.mtx);
}
static void slot_set_state(Slot& s, SlotState ns) {
    pthread_mutex_lock(&s.mtx);
    s.state = ns;
...
struct Metrics {
    ...
    vector<double> s1_pure_conv;   // NEW: 순수 GPU conv 시간 (통제 변수)
...
static void stage1_thread(Metrics* m, cudaStream_t stream1) {
    set_affinity({1});   // NEW: 코어 1 고정

    // NEW: 순수 GPU conv 시간 측정용 cudaEvent (통제 변수)
...
static void stage2_thread(Metrics* m) {
    set_affinity({2,3,4,5});   // NEW: 코어 2-5 고정 (OpenMP 4스레드용)
    m->ready.fetch_add(1);
    while (m->ready.load() < 3) this_thread::yield();
    ...
    for (int fid = 0; fid < NUM_FRAMES; fid++) {
        ...
        auto tw1 = Clock::now();
        auto tr0 = Clock::now();
        run_fc_cpu_omp(g_slots[s].pool3_buf);   // CHANGED: OpenMP 버전
        auto tr1 = Clock::now();
